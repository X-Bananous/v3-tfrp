...
export const showGlobalAudit = async (charId) => {
    const char = state.characters.find(c => c.id === charId) || state.allCharactersAdmin?.find(c => c.id === charId);
    if (!char) return;

    ui.showToast("Génération de l'audit en cours...", "info");
    ...
    try {
        // Récupération multi-sources avec allSettled pour ne pas bloquer si une table échoue
        const [bankRes, invRes, reportsRes] = await Promise.allSettled([
            ...
            state.supabase.from('police_report_suspects').select('id', { count: 'exact', head: true }).eq('character_id', charId)
        ]);

        if (bankRes.status === 'fulfilled') bankData = bankRes.value.data;
        if (invRes.status === 'fulfilled') invData = invRes.value.data || [];
        if (reportsRes.status === 'fulfilled') reportsCount = reportsRes.value.count || 0;
        ...
        content: `
            <div class="space-y-6 text-left">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-gov-light p-4 rounded-2xl border border-gray-100">
                        <div class="text-[8px] text-gray-400 font-black uppercase tracking-widest mb-1">Passif Judiciaire</div>
                        <div class="text-lg font-mono font-black text-red-600">${reportsCount} Rapport(s)</div>
                    </div>
                </div>

                <div>
                    <div class="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-2 ml-1">Inventaire Physique (${invData.length} types d'objets)</div>
                    <div class="bg-gov-light p-4 rounded-2xl border border-gray-100 max-h-40 overflow-y-auto custom-scrollbar">
                        ${invData.length > 0 ? invData.map(i => `
                            <div class="flex justify-between items-center border-b border-gray-200/50 py-1.5 last:border-0">
                                <span class="text-xs font-bold text-gov-text uppercase">${i.name}</span>
                                <span class="text-[10px] font-mono font-bold text-gray-400">x${i.quantity}</span>
                            </div>
                        `).join('') : '<div class="text-xs text-gray-400 italic py-2">Sac vide ou données inaccessibles.</div>'}
                    </div>
                </div>

                <div class="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                    <div class="text-[8px] text-blue-600 font-black uppercase tracking-widest mb-1">Note de l'Auditeur</div>
                    <p class="text-[10px] text-blue-800 leading-relaxed italic">Cet audit consolide les données bancaires, les registres d'inventaire et les archives du CAD national à l'instant T.</p>
                </div>
            </div>
        `,
