...
export const ProfileHubView = () => {
    ...
    const charGrid = characters.map(char => {
        ...
        return `
            <div onclick="${isAccepted ? `actions.selectCharacter('${char.id}')` : ''}" class="gov-card p-8 flex flex-col h-[400px] bg-white animate-in relative group overflow-hidden">
    ...
    <div class="flex-1 flex flex-col bg-[#F6F6F6] min-h-screen overflow-y-auto custom-scrollbar">
            ...
            <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
                <div class="flex items-center gap-8 text-center md:text-left">
                    ...
                    <div>
                        ...
                        <p class="text-gray-400 font-mono text-[10px] uppercase tracking-widest mt-1">ID Unique : ${u.id}</p>
                    </div>
                </div>
            ...
            <div class="max-w-6xl mx-auto space-y-16 pb-20">
                ...
                <div class="space-y-8">
                    <h3 class="text-xs font-black text-gov-text uppercase tracking-[0.4em] flex items-center gap-4 px-2">
                        <span class="w-12 h-0.5 bg-gov-blue"></span> Vos Dossiers (${characters.length}/${CONFIG.MAX_CHARS})
                    </h3>
                ...
                <!-- INFOS SYSTÈME ET SÉCURITÉ -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div class="gov-card p-10 bg-white shadow-xl cursor-default hover:transform-none">
                    ...
                    <div class="gov-card p-10 bg-white shadow-xl border-t-8 border-red-600 cursor-default hover:transform-none">
                        <h4 class="text-[10px] font-black text-red-600 uppercase tracking-[0.3em] mb-6">Sécurité du Compte</h4>
                        <p class="text-xs text-gray-500 mb-10 leading-relaxed font-medium">Action irréversible : Purger l'intégralité de vos dossiers citoyens, comptes bancaires et inventaires (RGPD).</p>
                        <button onclick="actions.requestDataDeletion()" class="w-full py-4 bg-red-600/10 text-red-600 font-black text-[10px] uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all border border-red-200">
                            Supprimer mes données (Purge 72h)
                        </button>
                    </div>
                </div>
        ...
        <div class="bg-white border-t border-gray-100 py-8 text-center shrink-0">
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-[0.6em]">Terminal de Gestion Citoyenne • Los Angeles Division</p>
        </div>
    </div>
    `;
};
