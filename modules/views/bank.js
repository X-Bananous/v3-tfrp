...
export const BankView = () => {
    if (!state.bankAccount) return '<div class="p-8 text-center text-gray-500 flex flex-col items-center justify-center h-full"><div class="w-10 h-10 border-4 border-gov-blue border-t-transparent rounded-full animate-spin mb-4"></div>Synchronisation du Terminal Financier...</div>';
    ...
    const historyHtml = state.transactions.length > 0 
        ? state.transactions.map(t => {
            ...
            return `
                <div class="flex items-center justify-between p-5 bg-white rounded-[24px] border border-gray-100 hover:shadow-xl transition-all group">
        ...
        : '<div class="text-center text-gray-400 py-16 opacity-30 flex flex-col items-center"><i data-lucide="history" class="w-12 h-12 mb-4"></i><span class="text-[10px] font-black uppercase tracking-[0.4em]">Aucun flux récent</span></div>';

    return `
        <div class="h-full flex flex-col bg-[#F8FAFC] overflow-hidden animate-fade-in">
            <!-- HEADER : Richesse Consolidée -->
            <div class="px-6 md:px-12 pt-8 pb-6 shrink-0 flex flex-col md:flex-row justify-between items-end gap-6 bg-white border-b border-gray-100 shadow-sm">
                <div>
                ...
                <div class="text-right">
                    <div class="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1">Valeur Nette Totale</div>
            ...
            <!-- DASHBOARD UNIQUE : Tout par scroll vertical -->
            <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
                <div class="max-w-7xl mx-auto space-y-12 pb-32">
                    
                    <!-- SECTION 1 : Comptes Courants & Carte Métal -->
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <!-- THE METAL CARD -->
                        <div class="relative group">
                            <div class="p-10 rounded-[48px] bg-gradient-to-br from-[#1e293b] via-[#0f172a] to-black relative overflow-hidden shadow-2xl border border-white/5 flex flex-col justify-between min-h-[320px]">
                                ...
                                <div class="relative z-10 my-8">
                                    <div class="text-[10px] text-blue-200/40 uppercase font-black tracking-[0.3em] mb-3">Solde Courant</div>
                        ...
                        <!-- ATM ACTIONS -->
                        <div class="bg-white p-10 rounded-[48px] border border-gray-100 shadow-xl flex flex-col justify-between relative overflow-hidden group">
                            ...
                            <div class="grid grid-cols-2 gap-6 relative z-10 pt-10 border-t border-gray-50">
                                <form onsubmit="actions.bankDeposit(event)" class="space-y-4">
                                    <div class="relative">
                                        ...
                                        <input type="number" name="amount" placeholder="Verser" class="w-full pl-8 pr-4 py-3 rounded-2xl text-sm font-mono font-black bg-gray-50 border-none focus:ring-4 focus:ring-emerald-500/10 transition-all" max="${cashBalance}">
                                    </div>
                                    <button type="submit" class="w-full py-4 rounded-2xl bg-gov-text text-white text-[10px] font-black uppercase tracking-[0.3em] hover:bg-emerald-600 transition-all shadow-xl shadow-gray-900/10">Dépôt Guichet</button>
                                </form>
                                <form onsubmit="actions.bankWithdraw(event)" class="space-y-4">
                                    <div class="relative">
                                        ...
                                        <input type="number" name="amount" placeholder="Retirer" class="w-full pl-8 pr-4 py-3 rounded-2xl text-sm font-mono font-black bg-gray-50 border-none focus:ring-4 focus:ring-red-500/10 transition-all" max="${bankBalance}">
                                    </div>
                                    <button type="submit" class="w-full py-4 rounded-2xl bg-gov-text text-white text-[10px] font-black uppercase tracking-[0.3em] hover:bg-red-600 transition-all shadow-xl shadow-gray-900/10">Retrait ATM</button>
                                </form>
                            </div>
                        </div>
                    </div>

                    <!-- SECTION 2 : Virement & Épargne side-by-side -->
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        <!-- TRANSFER MODULE -->
                        <div class="lg:col-span-7 bg-white p-12 rounded-[48px] border border-gray-100 shadow-xl relative overflow-hidden group">
                            ...
                            <form onsubmit="actions.bankTransfer(event)" class="space-y-8" autocomplete="off">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div class="space-y-3">
                                        <label class="text-[10px] text-gray-400 uppercase font-black tracking-widest ml-2 block">Note / Motif</label>
                                ...
                                <button type="submit" class="w-full py-5 rounded-[24px] font-black text-xs uppercase tracking-[0.4em] bg-gov-text hover:bg-blue-600 text-white shadow-2xl transition-all transform active:scale-[0.98]">EXÉCUTER LE TRANSFERT</button>
                            </form>
                        </div>

                        <!-- SAVINGS MANAGEMENT -->
                        <div class="lg:col-span-5 bg-[#020617] p-10 rounded-[48px] text-white shadow-2xl relative overflow-hidden flex flex-col justify-between group">
                            ...
                            <div class="flex justify-between items-start relative z-10 mb-10">
                                ...
                                <div class="text-right">
                                    <div class="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Taux Fixe</div>
                            ...
                            <div class="space-y-4 relative z-10">
                                <form onsubmit="actions.transferToSavings(event)" class="flex gap-3">
                                    <input type="number" name="amount" placeholder="Alimenter..." min="1" max="${bankBalance}" class="flex-1 bg-white/5 border border-white/10 p-4 rounded-2xl text-sm font-mono font-bold text-white outline-none focus:ring-2 focus:ring-emerald-500/50" required>
                                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-500 text-white px-6 rounded-2xl shadow-xl transition-all" title="Dépôt Épargne"><i data-lucide="plus" class="w-5 h-5"></i></button>
                                </form>
                                <form onsubmit="actions.withdrawFromSavings(event)" class="flex gap-3">
                                    <input type="number" name="amount" placeholder="Récupérer..." min="1" max="${savingsBalance}" class="flex-1 bg-white/5 border border-white/10 p-4 rounded-2xl text-sm font-mono font-bold text-white outline-none focus:ring-2 focus:ring-blue-500/50" required>
                                    <button type="submit" class="bg-white/10 hover:bg-white/20 text-white px-6 rounded-2xl shadow-xl transition-all" title="Retrait Épargne"><i data-lucide="minus" class="w-5 h-5"></i></button>
                                </form>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-12 rounded-[48px] border border-gray-100 shadow-xl">
